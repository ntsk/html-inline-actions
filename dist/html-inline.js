...
async function inlineHtml(filePath, options = {}) {
    ...
    const basedir = (0, path_1.resolve)(filePath, '..');
...
async function inlineStyles(html, basedir) {
    ...
    while ((match = linkRegex.exec(html)) !== null) {
        ...
        if (hrefMatch) {
            ...
            try {
                const cssContent = await fs_1.promises.readFile(cssPath, 'utf-8');
...
async function inlineScripts(html, basedir) {
    ...
    while ((match = scriptRegex.exec(html)) !== null) {
        ...
        if (srcMatch) {
            ...
            try {
                const jsContent = await fs_1.promises.readFile(jsPath, 'utf-8');
...
async function inlineImages(html, basedir) {
    ...
    while ((match = imgRegex.exec(html)) !== null) {
        ...
        if (srcMatch) {
            ...
            try {
                const ext = (0, path_1.extname)(imgPath).replace(/^\./, '').toLowerCase();
                const mimeType = getMimeType(ext);
                const dataUrl = await (0, streaming_base64_1.createDataUrl)(imgPath, mimeType);
...
async function inlineLinks(html, basedir) {
    ...
    while ((match = linkRegex.exec(html)) !== null) {
        ...
        if (hrefMatch) {
            ...
            try {
                const linkBuffer = await fs_1.promises.readFile(linkPath);
                const ext = (0, path_1.extname)(linkPath).replace(/^\./, '').toLowerCase();
                const mimeType = getMimeType(ext);
...
function fixPath(p, basedir) {
    if ((0, path_1.isAbsolute)(p)) {
        return (0, path_1.resolve)(basedir, (0, path_1.relative)('/', p));
    }
    else {
